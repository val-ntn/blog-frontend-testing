...
export default function ReportRecycleControl({ refreshFlag, onRestore }) {
  ...
  const handleRestore = async (id) => {
    try {
      await axios.patch(
        `${API_BASE_URL}/reports/restore/${id}`,
        {},
  ...
  const handleHardDelete = async (id) => {
    try {
      await axios.delete(`${API_BASE_URL}/reports/hard/${id}`, {
        withCredentials: true,
      });
      if (onRestore) onRestore();
    } catch (err) {
        ...
        renderActions={(report) => (
          <div className="dashboard-action-buttons">
            <div className="dashboard-action-buttons__left">
              <button onClick={() => handleRestore(report._id)}>
                ♻ Restore
              </button>
              <button onClick={() => handleHardDelete(report._id)}>
                🗑 Hard Delete
              </button>
            </div>
